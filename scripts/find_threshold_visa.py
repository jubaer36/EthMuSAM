#!/usr/bin/env python3
"""Find optimal SegF1 threshold on VisA test split.

Calibrating on VisA (public, independent dataset) avoids any leakage from
the MVTec AD 2 competition test splits. Pass the resulting threshold to
...
def main():
    ...
    parser.add_argument("--data_path",      default="./data/visa/")
    ...
    print("\n" + "=" * 60)
    print("Inference on VisA test split")
    print("=" * 60)

    for category in args.classes:
        print(f"\n[{category}]")
        try:
            dataset = visa.VisaDataset(
    ...
    print(f"\n{'='*60}")
    print(f"Global threshold (VisA): {global_thr:.6f}")
    print(f"{'='*60}")
